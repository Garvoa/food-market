<template>
  <div class="attrButeList" ref="attrButeList">
    <div class="attrButehead">
      <div class="attrButeListhead">請選擇套餐項目</div>
      <!-- 点餐详情区域 -->
      <div class="row entree">
        ...
        <div
          class="mainfood col-md-4"
        >{{$route.query.item?$route.query.item.mainFood:$route.query.name}}</div>
        <span class="col-md-2 foodPrice">x{{$route.query.item?$route.query.item.num:1}}</span>
        <span
          class="col-md-2 foodPrice"
        >${{$route.query.item?$route.query.item.mainFoodPrice:$route.query.price}}</span>
        ...
        <div class="col-md-8 foodName">
          ...
          <ul class="attrButeDetailsList row">
            <li v-for="(item,index) in attrButeDetails" :key="index">
              <span class="col-md-6">{{item.name}} {{item.attrButename}}</span>
              <span class="col-md-3">x{{item.num}}</span>
              <span class="col-md-3">￥{{item.price}}</span>
            </li>
          </ul>
        </div>
        <div class="price" style="font-size:30px">
          總金額 ￥{{$route.query.item
          ? $route.query.item.mainFoodPrice* $route.query.item.num+totalAmount:$route.query.price*1+totalAmount}}
        </div>
      </div>
    ...
    <!-- 风格设置，选择配菜区域 -->
    <!-- <AttrButeModule /> -->
    <van-collapse v-model="activeName" accordion class="attributeList-content">
      <van-collapse-item title="标题1" name="1">内容</van-collapse-item>
      <van-collapse-item title="标题2" name="2">内容</van-collapse-item>
      <van-collapse-item title="标题3" name="3">内容</van-collapse-item>
    </van-collapse>
    <van-button>退出</van-button>
    <van-button>確定</van-button>
  </div>
</template>

<script>
// import AttrButeModule from '../../components/AttrButeModule'
      ...
      if (!this.$route.query.isUpdate) {
        ...
        window.localStorage.setItem(
          ...
          JSON.stringify({
            totalAmount:
              this.totalAmount +
              this.$route.query.item.mainFoodPrice * this.$route.query.item.num,
  ...
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      if (!!vm.$route.query.isUpdate) {
        ...
        vm.totalAmount =
          vm.$route.query.item.totalAmount -
          vm.$route.query.item.mainFoodPrice * vm.$route.query.item.num
        window.localStorage.setItem(
          'attrButeDetailsList',
          JSON.stringify(vm.$route.query.item)
        )
      } else {
        vm.id = parseInt(
          Date.now() / Math.floor(Math.random() * 20202020 + 20212021)
